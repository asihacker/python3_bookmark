...
# 如何在字符串前补零
g = -42
# 可以用{expr:0len} 这个方法来进行字符串补零。len是最终返回字符串的长度。
# 还可以增加一个正负号标记。在这种情况下，用+则无论数值为正负数都将显示相应正负号。用-则只有当数值为负数时才显示负号，默认设定也是如此。
print(f'{g:-011}')
print(f'{g:+011}')
